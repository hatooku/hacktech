...
TOUCHING_DISTANCE = 6.5 # resolution of screen / width of phone past detection
...
def isCollinear(p1, p2, p3):
    if p1[0] != p2[0] and p1[0] != p3[0]:
        ...
        if abs(slope1 - slope2) <= CELL_ERROR:
            return True
    else:
        if (abs(p1[0] - p2[0]) <= CELL_ERROR and abs(p1[0] - p3[0]) <= CELL_ERROR):
            return True
    return False
        ...
        for j in range(0, k):
            if k != j:
                ...
                # 2nd octant of plane
                    ...
                    x = p1[0];
                    for i in range(int(p1[1]), int(p2[1]) + 1):
                        points1.append((x, i))
                ...
                # 5th octant of plane
                    ...
                    y = p2[1];
                    for i in range(int(p2[0]), int(p2[0]) + 1):
                        points1.append((i, y))
                        error -= deltay
                        if (2 * error >= -deltax):
                ...
                # 6th octant of plane, increment by y
                    ...
                    x = p2[0];
                    for i in range(int(p2[1]), int(p1[1]) + 1):
                        points1.append((x, i))
                        error -= deltax; # increase error
                ...
                # 4th octant of plane, increment by x
                    ...
                    y = p2[1];
                    for i in range(int(p2[0]), int(p1[0]) + 1):
