        ...
        while True:
            ...
            if p[0] == 2: # image packet
                ...
                if self.baseline is None:
                    ...
                    pixels = self.processPixels(pixels)    
                img = { 'timeStamp' : p[5] + (p[6] << 16),
                        'sequence' : p[10],
                        'rows' : rows,
                        'cols' : cols,
...
class MyHttpHandler(httpServer.BaseHTTPRequestHandler):
    ...
    def send_sensorData(self):
        ...
        if len(images) > 0:
            ...
            print(cluster.findClusterCenters(images[-1]["image"]))
        else:
            if VERBOSE:
                print("Warning: no image available")
            self.writeStr("{}")
    
    def send_rezero(self):
        global sensor
        self.send_response(200)
        self.send_header("Content-type", "application/json")
        self.end_headers()
        images = sensor.setBaseline()
        if len(images) > 0:
            self.imageToJSON(images[-1])
        else:
            if VERBOSE:
